 ...
 * rendered in {@code opt_wrapper} element or element with id="root".
 ...
 * @constructor
 */
zz.controllers.FEBase = function( dataset, view, opt_dom, opt_wrapper, opt_root ){
     ...
     * @deprecated
     */
    this.wrapperElementInternal = opt_wrapper ||
     ...
     * @private
     */
    this.rootController_ = undefined;
...
/**
 * Determine is current controller root or no. Couldn't be used
 * in child constructor. Could be used starting from
 * {@code #setupListenersInternal} phase.
 * @returns {boolean}
 */
...
/**
 * Return root controller. Couldn't be used
 * in child constructor. Could be used starting from
 * {@code #setupListenersInternal} phase.
 * @returns {zz.controllers.FEBase}
 */
zz.controllers.FEBase.prototype.getRootController = function( ){
...
zz.controllers.FEBase.prototype.enterDocument = function( ){
    ...
    if( this.isRoot( ) ){
        ...
        if( this.getParent( ) ){
            ...
            throw new Error( 'Root controller can not have parent controller.' );
        }
        this.rootController_ = this;
        ...
        if( !root ){
            ...
            throw new Error( 'Root controller is not defined.' );
        }
        this.rootController_ = root;
    }
...
/**
 * Enable change event for contenteditable elements for
 * IE9+ (fixing IE bug).
 * @param {Element} element
 */
zz.controllers.FEBase.prototype.enableIEContenteditableChangeEvent = function( element ){
    ...
    this
        ...
        .listen(
            ...
            element,
            goog.events.EventType.DOMCHARACTERDATAMODIFIED,
            this.handleClientEvent );
};
...
zz.controllers.FEBase.prototype.handleClientEvent = function( e ){
    ...
    var target = e.type === goog.events.EventType.DOMCHARACTERDATAMODIFIED ?
        e.currentTarget:
        e.target;

    var uid;
    if( uid = this.renderer_.getElementUid( target ) ){
        ...
        if( !node ){
            ...
            switch( e.type ){
                ...
                case goog.events.EventType.DOMCHARACTERDATAMODIFIED:

                    if( goog.labs.userAgent.browser.isIE( ) &&
                        goog.isDef( target.parentElement.getAttribute( 'contenteditable' ) ) ){
...
zz.controllers.FEBase.prototype.handleChange = function( e ){
    ...
    var uid = this.renderer_.getElementUid( target );
    var node = this.mvcRegistry_.get( uid );
    var field = this.getView( ).getFieldByUid( uid );
    var oldVal = node.model[ field ];
    var newVal = this.renderer_.getElementValue( target );
    if( oldVal !== newVal ){
