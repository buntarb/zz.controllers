 ...
 * @constructor
 */
zz.controllers.FEBase = function( dataset, view, opt_rootElement, opt_dom ){
    ...
    if( this.isRoot_ ){
        ...
        this.render( opt_rootElement || goog.dom.getElement( 'root' ) );
    }
};
...
zz.controllers.FEBase.prototype.disposeInternal = function( ){
    ...
    this.mvcTree_.deleteNode( this.model_.getUid( ).toString( ) );
    this.model_.dispose( );
    if( !this.isRoot_ ){
 ...
 * logic could be runned. Should be overriden in child class. Do nothing by default.
 */
zz.controllers.FEBase.prototype.bootstrap = function( ){ };

/**
 * Add child controller and render in into {@code opt_wrapper} element if specified,
 * controller root element otherwise.
...
zz.controllers.FEBase.prototype.handleChange = function( e ){
    ...
    var uid = this.renderer_.getElementUid( e.target );
    var node = this.mvcTree_.getNode( uid );
    var field = this.mvcTree_.getFieldByUid( uid );
